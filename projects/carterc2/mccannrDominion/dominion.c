...
//Sea Hag
//Bug Notes:
// 		Original Line:	state->deck[i][state->deckCount[i]--] = curse;//Top card now a curse
// 		New Line: 		state->deck[i][state->deckCount[i--]] = curse;//Top card now a curse
// 		Bug: replaced 'deckCount[i]--' to 'deckCount[i--]'
...
// 		New Line: 'state->numActions;' 
// 		Bug: state->numActions no longer incremented 
...
//Adventurer
//Bug Notes:
// 		Original Lines: "temphand[z]=cardDrawn;" in the else statement of the while loop 
// 		New Line: "temphand[z]==cardDrawn;"  
// 		Bug: changed from an assignment to an equality check. 
// 		 
// Notes: already technically contains a (dominion) bug but wont arise with the set of cards supported by the program. 
// 					If player trashed all of their money, the while(drawntreasure < 2) loop would be infinite. 
int play_adventurer(struct gameState *state, int currentPlayer, int *temphand, int z, int drawntreasure){
	while(drawntreasure < 2){//int temphand[MAX_HAND];// moved above the if statement
...
//Smithy
//Bug Notes:
// 		Original Line: 'for (int i = 0; i < 3; i++)' 
// 		New Line: 'for (int i = 0; i <= 3; i++)' 
// 		Bug: loop will run until i <= 3 instead of i < 3 (so will run one extra time)  
int play_smithy(int currentPlayer, struct gameState *state, int handPos){
	for (int i = 0; i <= 3; i++)	drawCard(currentPlayer, state);//+3 Cards
	discardCard(handPos, currentPlayer, state, 0);//discard card from hand
	return 0; 
}
	...
	switch( card ) 
	{
		case adventurer:
			/*	while(drawntreasure<2){
				...
				return 0;
				*/
			return play_adventurer(state, currentPlayer, temphand, z, drawntreasure); 
