/*
 * Card Test for the "council_room" card.
 */
...
#include "rngs.h"
 ...
 int main() {
	...
	int p = 0;
	int supplyPass = 1;
	int r, expectedNumCards, actualNumCards, expectedNumBuys, actualNumBuys, expectedNumActions, actualNumActions;
	...
	if (actualNumCards == expectedNumCards) {
		printf("PASS: The 4 cards was gained correctly and 1 played from the hand, expected %d cards, actual %d cards for player %d.\n", expectedNumCards, actualNumCards, p);
	} else {
		printf("FAIL: Player has the incorrect amount of cards in hand, expected %d cards, actual %d cards for player %d.\n", expectedNumCards, actualNumCards, p);
		allPass = 0;
	}
	
	// Verify deck has 4 less cards
	expectedNumCards = baseG.deckCount[p] - 4;
	...
	// Verify that the other players deck reduced by 1, hand increased by 1, and discard did not change
	p = 1;
	expectedNumCards = baseG.deckCount[p] - 1;
	actualNumCards = G.deckCount[p];
